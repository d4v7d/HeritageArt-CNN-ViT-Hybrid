"""
Training script for POC-5: Multi-backbone UPerNet Comparison
Trains ConvNeXt, Swin, or CoaT with UPerNet decoder
"""
...
def compute_metrics(pred, target, ignore_index=255, num_classes=2):
    ...
    if len(pred_masked) == 0:
        return {'miou': 0.0, 'mf1': 0.0, 'accuracy': 0.0}
...
def train_epoch(model, loader, criterion, optimizer, device, epoch, total_epochs, scaler=None):
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description='Train multi-backbone UPerNet model')
    parser.add_argument('--config', type=str, required=True,
                       help='Path to model config file (convnext/swin/coat)')
    ...
    # Merge configs (model_config overrides base_config)
    ...
    print(f"  Encoder weights: {config['model'].get('encoder_weights', 'None')}")
    ...
    checkpoint_dir = Path('logs') / model_name / 'checkpoints'
    logs_dir = Path('logs') / model_name / 'training'
    ...
    csv_writer.writerow([
        'epoch', 'train_loss', 'train_miou', 'train_mf1', 'train_acc',
        'val_loss', 'val_miou', 'val_mf1', 'val_acc', 'lr'
    ])
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        csv_writer.writerow([
            epoch,
            train_loss, train_metrics['miou'], train_metrics['mf1'], train_metrics['accuracy'],
            val_loss, val_metrics['miou'], val_metrics['mf1'], val_metrics['accuracy'],
        ...
        if epoch % config['training']['checkpointing']['save_freq'] == 0 or is_best:
            checkpoint = {
                'epoch': epoch,
