...
class HierarchicalUPerNet(nn.Module):
    ...
    def __init__(self, encoder_name, encoder_weights, in_channels_list,
        ...
        self.use_smp_encoder = use_smp_encoder
    ...
    def forward(self, x, return_all_heads=True):
        ...
        # Handle SMP encoder output (includes input feature)
        if self.use_smp_encoder:
            # SMP returns [input, stage1, stage2, stage3, stage4]
            # We need last 4: [stage1, stage2, stage3, stage4]
...
def build_hierarchical_model(config):
    ...
    ENCODER_CHANNELS = {
        ...
        'mit_b3': [64, 128, 320, 512],  # SegFormer MiT-B3 (from SMP)
    }
    ...
    if in_channels_list is None:
        ...
        # Try SMP first (for models like mit_b3)
        if encoder_name in smp.encoders.get_encoder_names():
            print(f"   Using SMP encoder: {encoder_name}")
            dummy_encoder = smp.encoders.get_encoder(
                encoder_name,
                in_channels=3,
                depth=5,
                weights=None
            ...
            # SMP returns features including input, we need last 4
            in_channels_list = [f.shape[1] for f in features[-4:]]
            print(f"   Detected (SMP): {in_channels_list}")
            del dummy_encoder
        else:
            # Fall back to timm
            print(f"   Using timm encoder: {encoder_name}")
            encoder_kwargs = {
                'pretrained': False,
                'features_only': True,
                'out_indices': (0, 1, 2, 3)
            ...
            print(f"   Detected (timm): {in_channels_list}")
            del dummy_encoder
